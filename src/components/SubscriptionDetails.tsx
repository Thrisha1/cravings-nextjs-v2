...
const SubscriptionDetails = ({ subscription }: { subscription: Subscription | null }) => {
  ...
  const addSubscription = (paymentData: any) => {
    if (!subscription) {
    ...
    // Here you would typically send these records to your backend API
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle>
          {isPaid ? "Payment Successful" : qrCodeData ? "Complete Your Payment" : "Subscription Details"}
        </CardTitle>
        <CardDescription>
          {isPaid ? "Your subscription is now active." :
           qrCodeData ? `Scan the QR code to renew for ₹${subscription.plan}` :
      ...
      <CardContent className="grid gap-4">
        ...
        {isPaid ? (
          <div className="flex flex-col items-center gap-4 text-center py-8">
            ...
            <p className="text-sm text-gray-500">Your subscription has been renewed.</p>
          </div>
        ) : qrCodeData ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-center text-gray-500 animate-pulse">
             ...
             <div className="flex items-center gap-4">
                <a href={qrCodeData.link.gpay} className="text-blue-600 hover:underline">GPay</a>
                <a href={qrCodeData.link.phonepe} className="text-purple-600 hover:underline">PhonePe</a>
                <a href={qrCodeData.link.paytm} className="text-blue-500 hover:underline">Paytm</a>
            </div>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium">₹{subscription.plan} / {subscription.type}</span>
            </div>
            <div className="flex items-center justify-between">
