...
interface OrderState {
  ...
  fetchCaptainOrders: (partnerId: string) => Promise<Order[] | null>;
...
const useOrderStore = create(
  persist<OrderState>(
    (set, get) => ({
      ...
      fetchCaptainOrders: async (partnerId: string) => {
        try {
          // First fetch the orders with captain data included
          const ordersResponse = await fetchFromHasura(
            `query GetCaptainOrders($partnerId: uuid!) {
              orders(
                where: { partner_id: { _eq: $partnerId }, orderedby: { _eq: "captain" } }
                order_by: { created_at: desc }
              ) {
                id
                total_price
